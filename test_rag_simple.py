"""
Test RAG pipeline with smaller models
"""
...
def test_rag_with_simple_models():
    """Test RAG pipeline with simpler models"""
    try:
        logger.info("Testing RAG pipeline with simple models...")
        ...
        # Override model names to use smaller models
        pipeline.model_loader._llama_model = None  # Reset
        
        # Test with text-only query first (should use Llama)
        result = pipeline.answer_question(
            query="What is the speed of the ego vehicle?",
            scene_id=1,
            keyframe_id=1,
            force_strategy="text_only"  # Force text-only to avoid LLaVA
        )
...
def main():
    ...
    success = test_rag_with_simple_models()
